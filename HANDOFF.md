...
| **P2-5** scale handle / rotate + Shift 15° | ← **เริ่มที่นี่** ดู §2.4 |

**เกณฑ์คุณภาพล่าสุดที่ผ่าน:** `fmt` / `clippy --all-features -D warnings` (workspace + `fuzz/`) /
`nextest 390/390` / `deny check` ครบ 4 หมวด / **binary 15.59 MB จากเพดาน 25 MB (บังคับใน CI)**
...
### 2.4 ★ P2-5 ส่วนที่เหลือ: scale handle · rotate

**ส่วนย้ายเสร็จแล้ว** (`d020219`) — `TransformItems` + merge/seal มีคนเรียกแล้ว
ที่เหลือคือ handle ซึ่ง `docs/03 §2` ระบุพฤติกรรมไว้ครบ:

| | ทำอะไร | modifier |
|---|---|---|
| Scale | ลาก handle **มุม** | `Shift` = คงสัดส่วน · `Alt` = ยืดจากจุดกึ่งกลาง |
| Rotate | ลาก **นอก** handle มุม | `Shift` = สแนป 15° |

สิ่งที่ต้องคิดตอนทำ:

- handle ต้องมีขนาดคงที่ **บนจอ** ไม่ใช่ใน world ไม่งั้นซูมออกแล้วจับไม่โดน
  (คำนวณเหมือน `drag_threshold` คือ พิกเซล ÷ zoom)
- hit-test ของ handle ต้องมาก่อน hit-test ของภาพ ไม่งั้นลาก handle จะกลายเป็นย้าย
- ภาพที่หมุนแล้ว handle ต้องหมุนตาม — ใช้ `ItemCanvas::obb().corners()` ที่มีอยู่แล้ว
- เลือกหลายใบพร้อมกันแล้ว scale ควรทำกับกรอบรวม ไม่ใช่ทีละใบ (ตัดสินตอนทำ)
