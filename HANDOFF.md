...
| **P2-9** align / distribute + ไกด์ | ⚠️ **ตรรกะครบ แต่ยังไม่ยืนยันบนของจริง** ดู §2.8 ← **เริ่มที่นี่** |

**เกณฑ์คุณภาพล่าสุดที่ผ่าน:** `fmt` / `clippy --all-features -D warnings` (workspace + `fuzz/`) /
`nextest 488/488` / `deny check` ครบ 4 หมวด / **binary 15.83 MB จากเพดาน 25 MB (บังคับใน CI)**
...
### 2.8 ⚠️ P2-9: align / distribute + ไกด์ — **ตรรกะครบ แต่ยังไม่ยืนยันบนของจริง**

> ★★ **อย่าเชื่อว่ามันทำงาน** — เทสต์ 15 ตัวใน `refx_core::align` ผ่านหมด
> แต่นั่นคือหลักฐานว่า**แต่ละชิ้นถูก ไม่ใช่ว่าประกอบถูก** ซึ่งเป็นบั๊กชนิดที่สาม
> ใน `docs/08 §3.9` ข้อ 5 ที่ **มีแต่การรันของจริงเท่านั้นที่จับได้**

#### ★ สามข้อที่ค้าง — ต้องทำก่อนบอกว่า P2-9 เสร็จ

1. **ไกด์ตอนลาก ยังไม่มีภาพหน้าจอพิสูจน์** — ลากทดสอบในแอปแล้ว **ภาพไม่ขยับเลย**
   (ขอบซ้ายอยู่ที่ x=659 ทั้งก่อนและหลังลาก 28 px) แปลว่าการกดนั้น
   **ไม่ได้ลงทะเบียนเป็น `Press`** · ยังแยกไม่ได้ว่าเป็นปัญหาของสคริปต์ขับเมาส์
   (`scratchpad/drive.ps1` ต้องให้เฟรมนิ่งก่อนกด — เคยเจอมาแล้วตอน P2-5)
   หรือของโค้ดเอง → **ไล่ข้อนี้ก่อนอย่างอื่น**
2. **align / distribute ยังไม่ได้กดทดสอบในแอปเลยสักปุ่ม** — ปุ่มแปดตัวขึ้นบน
   toolbar แล้วและอ่านออก แต่ยังไม่มีใครยืนยันว่ากดแล้วภาพขยับถูกทาง
3. **ยังไม่ได้วัด 1000 ภาพแล้วลาก** — เทสต์ `finding_guides_looks_at_neighbours_not_the_whole_board`
   พิสูจน์แค่ว่า **index คืนเพื่อนบ้าน < 20 ใบจาก 1000** ไม่ได้พิสูจน์ว่าเฟรมไม่ตกจริง
...
| เรื่อง | หมายเหตุ |
|---|---|
| **★ P2-9 ยังไม่ยืนยันบนของจริง** | สามข้อค้างอยู่ใน §2.8 — ไกด์ยังไม่มีภาพหน้าจอ · align/distribute ยังไม่ได้กดทดสอบ · ยังไม่ได้วัด 1000 ภาพ · **อาการที่ต้องไล่ก่อน:** ลากในแอปแล้วภาพไม่ขยับ (659 → 659) แปลว่า `Press` ไม่ลงทะเบียน |
| **สคริปต์ขับเมาส์ต้องให้เฟรมนิ่งก่อนกด** | `scratchpad/drive.ps1` — `move` แล้วต้อง `sleep` ก่อน `down` ไม่งั้น egui รวม event เข้าเฟรมเดียวแล้ว `Press` ไปตกที่ตำแหน่งหลังขยับ (เจอครั้งแรกตอน P2-5) · เป็นข้อสงสัยอันดับแรกของอาการข้างบน |
