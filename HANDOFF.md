...
| **P2-7** crop แบบ non-destructive | ← **เริ่มที่นี่** (`crop` ยังไม่ถึง shader — §2.2d) |

**เกณฑ์คุณภาพล่าสุดที่ผ่าน:** `fmt` / `clippy --all-features -D warnings` (workspace + `fuzz/`) /
`nextest 438/438` / `deny check` ครบ 4 หมวด / **binary 15.59 MB จากเพดาน 25 MB (บังคับใน CI)**
