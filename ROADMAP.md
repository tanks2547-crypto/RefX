...
> ### ★★★ สิ่งเดียวที่บล็อก AppImage: **โปรเจกต์นี้ไม่มีไอคอนเลย** (14 ก.ย. 2026)
>
> ไม่มี `.ico` · ไม่มี `.png` · ไม่มีการตั้ง window icon ในโค้ด ·
> `packaging/refx.desktop` ชี้ไปที่ `Icon=refx` ที่ยังไม่มีไฟล์รองรับ
>
> | รูปแบบ | ผลของการไม่มีไอคอน |
> |---|---|
> | zip · `.deb` · MSI | ใช้ได้ แต่แถบงาน/เมนู/Add-Remove Programs ขึ้นไอคอนกลาง ๆ ของ OS |
> | **AppImage** | **สร้างไม่ได้เลย** — `appimagetool` บังคับต้องมีไอคอนใน AppDir |
>
> ★ **ไม่ให้ agent ประดิษฐ์เอง** — หน้าตาของโปรแกรมเป็นการตัดสินใจของเจ้าของ
> · วันที่มีไฟล์จริง: วางที่ `usr/share/icons/hicolor/256x256/apps/refx.png`
> แล้วเติมเข้า `[package.metadata.deb] assets` — AppImage จะทำได้ทันที
>
...
| **d** 🔄 | แพ็กเกจ MSI / zip / AppImage / deb | ปลายทาง · zip + MSI + deb ทำแล้ว · **AppImage ติดที่ไอคอน** |
