...
## v1.1 — รายการที่ตัดสินแล้วว่า **ไม่บล็อก v1.0** (รวบรวม 19 ก.ย. 2026)

ทุกข้อในนี้ถูกไล่จนรู้ขอบเขตแล้ว และเลือกเลื่อนโดยมีเหตุผล **ไม่ใช่ของที่ลืม**

| # | เรื่อง | ทำไมเลื่อนได้ |
|---|---|---|
| 1 | **AppImage** | zip + MSI + deb ครอบคลุมผู้ใช้ส่วนใหญ่ · ไอคอนไม่ใช่อุปสรรคแล้ว เหลือ job + ประตูสองบานของตัวเอง |
| 2 | **`AssetRef.format` เป็น `Unknown` บนเส้นทาง cache hit** | ต้องเพิ่มคอลัมน์ใน cache DB = **เปลี่ยน schema** ซึ่งใกล้วันปล่อยมีความเสี่ยงมากกว่าประโยชน์ · v1.0 ซ่อนช่องนั้นไปเลยตามกฎ "ไม่รู้ = ช่องหาย ไม่ใช่ขึ้น `?`" |
| 3 | Inspector **แก้ค่าได้** (X/Y · W/H · rotation) + `crop` · `lock` | v1.0 แสดงอย่างเดียว — ดู `docs/03 §1` |
| 4 | **P5-8 atlas dedupe** | เป็นการประหยัด ไม่ใช่การแก้ของที่พัง |
| 5 | `--help` กับคอนโซลบน Windows (`AttachConsole`) | เลี่ยงได้ด้วย redirect · Linux ไม่มีปัญหา |
| 6 | Library ไม่ขึ้นชื่อโฟลเดอร์ · ป้าย `RAM` กำกวม | ความชัดเจนของ UI ไม่ใช่ความถูกต้อง |
| 7 | **การกู้ device หลังเครื่องหลับจริง** ยังไม่เคยเจอของจริง | ทดสอบจริงแล้ว 18 ก.ย. — โปรแกรมรอด แต่ `generation` ยังเป็น 0 แปลว่าไดรเวอร์ไม่ได้ทำ device หาย · เส้นทางกู้จึงมีแต่การจำลองรองรับ (`KNOWN-LIMITATIONS` ข้อ 11) |
| 8 | **ตัวจับ I-1 ร้องตอนลากไฟล์ค้างเหนือหน้าต่าง** | วัดแล้ว idle = 0.12–0.17 % · `worst_quiet=6` จาก 32 → ไม่ใช่การวาดวน · ต้องพิสูจน์สมมติฐาน `HoveredFile` ด้วยการแยกเหตุผล `DragHover` (`KNOWN-LIMITATIONS` ข้อ 12) |
| 9 | **`version_in_deb_name` เลิกใช้แล้ว แต่บทเรียนยังไม่ครบ** | `deb_name_declares` รับเฉพาะ revision ที่เป็นตัวเลขล้วนตามที่ `cargo deb` สร้าง — ถ้าวันหนึ่งเปลี่ยนเครื่องมือแพ็ก ต้องตรวจข้อนี้ก่อน |

> ### ★★ ข้อ 2 คือข้อเดียวที่เป็น **ข้อมูลหาย** ไม่ใช่ความสะดวก
> เรารู้ format ตอนอ่านไฟล์ครั้งแรก แล้ว**ทิ้งมันไป** · หลังรันครั้งแรก
> cache hit คือเส้นทางหลัก → ผู้ใช้แทบไม่มีวันเห็นค่าที่ถูกต้องเลย
> **อนุมัติเปลี่ยน schema ใน v1.1** พร้อม migration ที่อ่าน DB เก่าได้
