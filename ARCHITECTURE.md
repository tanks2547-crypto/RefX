...
1. **I-1 ไม่วาดเมื่อไม่มีอะไรเปลี่ยน.** ไม่มี render loop แบบวนตลอด เรนเดอร์เฉพาะเมื่อมี input, animation ที่กำลังเล่น, หรือ texture โหลดเสร็จ (`ControlFlow::Wait` + `request_redraw()`)

   > ### ★★★ แก้ถ้อยคำ 18 ก.ย. 2026 — เดิมเขียนว่า **"Idle = 0% CPU"**
   > ถ้อยคำเดิม **วัดไม่ได้จริง** · ค่าที่วัดได้ตอนนิ่งสนิทอยู่ที่ **0.11–0.28 %
   > ของหนึ่งคอร์** มาตลอดทั้งโปรเจกต์ และไปอยู่บน **เธรดที่ไม่ใช่ของเรา** —
   > เธรดของเราตั้งชื่อครบ (`main` · `refx-io` · `refx-decode-*` · `tracing-appender`)
   > และไม่มีตัวไหนกินเพิ่มแม้แต่ ms เดียวใน 180 วินาที · ตัวที่กินคือเธรดไม่มีชื่อ
   > สี่ตัวที่อยู่ในสถานะ `Wait` (ไดรเวอร์กราฟิก / thread pool ของ OS — **ยังไม่ยืนยันว่าตัวไหน**)
   >
   > **เกณฑ์ที่วัดได้จริง ใช้ทั้งสี่ข้อ:**
   >
   > | # | เกณฑ์ | วิธีตรวจ |
   > |---|---|---|
   > | 1 | `ControlFlow::Wait` เท่านั้น · ห้าม `Poll` | คอมไพเลอร์ + review |
   > | 2 | **ตัวนับเฟรมไม่ขยับ**ขณะไม่มี input | แฮชแถบสถานะ ≥ 120 วิ |
   > | 3 | **`worst_quiet` < 32** ตลอดอายุโปรเซส | ตัวจับถาวร พิมพ์ตอนปิดโปรแกรม |
   > | 4 | CPU ของโปรเซสตอนนิ่ง **≤ 1.0 % ของหนึ่งคอร์** | วัด ≥ 120 วิ โดยไม่แตะหน้าต่าง |
   >
   > ข้อ 4 ตั้งจากค่าที่วัดได้จริง **บวกระยะเผื่อประมาณสามเท่า** — ไม่ใช่ศูนย์
   > เพราะศูนย์ไม่เคยเป็นค่าที่เครื่องจริงให้ได้ · **เกณฑ์ที่ไม่มีใครผ่านได้คือเกณฑ์
   > ที่ทุกคนเรียนรู้ที่จะเพิกเฉย** (`docs/08 §3.9 ข้อ 11`)
   >
   > ★ ข้อ 2 กับ 3 คือหัวใจ · ข้อ 4 เป็นตาข่ายรองที่จับกรณีที่ CPU ไปไหม้
   > อยู่นอกเส้นทางวาด ซึ่งข้อ 2 มองไม่เห็น
   >
   > ### ★★★ "ไม่มี input" หมายถึงอะไร — แก้นิยาม 19 ก.ย. 2026
   >
   > นิยามเดิมของตัวจับนับเฉพาะ **event ที่เป็นชิ้น** (`UserInput`/`TextureReady`)
   > ไม่นับ **สภาพ** ที่ตัวชี้อยู่เหนือหน้าต่าง · วัดแล้วด้วยการปล่อยนิ่ง 130 วินาที
   > ต่างกันตัวแปรเดียว:
   >
   ...
   > | **แถบเครื่องมือ** | **24** | **20** |
   >
   > และ **ไม่ได้เกิดจาก event ที่วิ่งเข้ามาซ้ำ ๆ** — ตัวนับชนิด event (`inputs=`
   > ในบรรทัดปิดโปรแกรม) บอกว่า `CursorMoved` มาแค่ **3 ครั้งใน 130 วินาที**
   > · คนขอวาดคือ egui เองผ่าน `repaint_delay`
   >
   > → **การชี้เมาส์ค้างคือการโต้ตอบ ไม่ใช่ idle** · เหตุผล `HoverRepaint`
   > จึงรีเซ็ตช่วงเงียบเหมือน `UserInput`
   >
   > ★★★ **กันคนแก้กลับ: นี่คือความแม่นของตัวจับ ไม่ใช่การผ่อนเกณฑ์**
   > ตัวเลขยังถูกนับแยกและพิมพ์ตอนปิดโปรแกรมเหมือนเดิมทุกประการ —
   > **ที่เปลี่ยนคือมันไม่ปลุกเสียงเตือน ไม่ใช่มันหายไปจากสายตา** ·
   > **ข้อ 2 ข้างบนยังบังคับเต็มเหมือนเดิม** (เฟรมต้องไม่ขยับตอนไม่มีใครแตะ)
   >
   > ★★ NC ที่บังคับไว้ในโค้ด: `the_alarm_still_fires_for_a_real_loop_with_no_pointer_in_the_window`
   > — ถ้าเผลอทำให้ทุกอย่างรีเซ็ต เทสต์ตัวนี้ต้องแดง · พิสูจน์แล้วว่าแดงจริง
   >
   > ### ★★★ **เคอร์เซอร์ที่อยู่ในหน้าต่าง = ผู้ใช้กำลังโต้ตอบ** ไม่ใช่ idle (19 ก.ย. 2026)
   >
   > วัดแล้ว ปล่อยนิ่ง 130 วินาที ต่างกันตัวแปรเดียว:
   >
   ...
   >
   > **ไม่ได้เกิดจาก event ที่วิ่งเข้ามาซ้ำ** — `CursorMoved` มาแค่ 3 ครั้งใน 130 วิ
   > คนขอวาดคือ **egui เอง** ผ่าน `repaint_delay` (ตรรกะ hover/tooltip)
   > · ~1 ครั้งทุก 5 วินาที ไม่ใช่ animation ต่อเนื่อง
   >
   > **คำตัดสิน:** นิยาม "input" ของตัวจับ**แคบเกินไป** — มันนับเฉพาะ event ที่เป็นชิ้น
   > ไม่นับ *สภาพ* ที่เคอร์เซอร์อยู่เหนือหน้าต่าง · การชี้เมาส์ค้างคือการโต้ตอบ
   >
   > → **สภาพ "ตัวชี้อยู่ในหน้าต่าง" ต้องรีเซ็ตช่วงเงียบ** เหมือนที่ `TextureReady` ทำ
   >   — `docs/08 §3.9 ข้อ 11`: **ตัวจับที่ร้องตอนทุกอย่างปกติ คือตัวจับที่ไม่มีใครอ่าน**
   > → ★ แต่ **ห้ามซ่อนตัวเลข** — ต้องนับแยกเป็นเหตุผลของตัวเอง (`HoverRepaint`)
   >   แล้วพิมพ์ตอนปิดโปรแกรมเหมือนเดิม · ที่เปลี่ยนคือมันไม่ปลุกเสียงเตือน
   >   **ไม่ใช่มันหายไปจากสายตา**
   > → ★★ เขียนไว้กันคนแก้กลับ: นี่คือ **ความแม่นของตัวจับ ไม่ใช่การผ่อนเกณฑ์**
   >   ข้อ 2 (เฟรมไม่ขยับตอนไม่มีใครแตะ) ยังบังคับเต็มเหมือนเดิม
