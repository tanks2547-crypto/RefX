...
### ★★★ ห้ามอ่าน exit code ผ่าน pipe — **ทุกชนิด** ไม่ใช่แค่ `| head`

เดิมข้อห้ามมีแค่ `| head` กับ `2>/dev/null` · แต่มันเกิดซ้ำมาห้ารูป **เหตุเดียวกัน:
exit code ที่อ่านได้ไม่ใช่ของคำสั่งที่ตั้งใจถาม**

| รูป | เกิดเมื่อ | ผล |
|---|---|---|
| `cmd \| head` | หลายครั้ง | ตัดหลักฐานทิ้ง |
| `cmd 2>/dev/null` | 13 ก.ย. 2026 | ค้าง 1 ชม. 28 นาทีโดยไฟล์ output ว่าง |
| `a && b \|\| echo "…"` | 20 ก.ย. 2026 | พิมพ์ข้อสรุปทั้งที่ `b` ไม่ได้รัน |
| `cargo clippy … \| tail && echo ok` | 21 ก.ย. 2026 (`54544a6`) | พิมพ์ `clippy ok` ทั้งที่ clippy แดง |
| `ls dir 2>/dev/null \| wc -l` + `pipefail` + `bash -e` | 21 ก.ย. 2026 (run `35564526229`, เจอ 24 ก.ย.) | ★ **ทิศกลับ** — exit ของ `ls` ที่ไม่มีใครสนใจ **ฆ่า**ขั้นใน CI ทิ้ง ล็อกมีแค่ `exit code 2` ไม่มีคำอธิบายสักคำ · fuzzer ไม่ได้รันทั้งสี่ target |

```bash
❌ cmd | tail && echo ok        # exit ของ tail ไม่ใช่ของ cmd
❌ cmd 2>&1 | head -10          # ตัดหลักฐานทิ้ง + exit ของ head
❌ cmd 2>/dev/null || echo 0     # กลืนเหตุผลของความล้มเหลวไปด้วย
✅ cmd > out.txt 2>&1; echo "exit=$?"; tail out.txt     # อ่าน $? ของ cmd ตรง ๆ
✅ if [ -d "$d" ]; then find "$d" -type f | wc -l; else echo 0; fi
   # ถามก่อนว่ามีไหม · ไม่ใช่นับแล้วกลืน error ของกรณีที่ไม่มี
```

★ ข้อสุดท้ายสำคัญใน CI เป็นพิเศษ: Actions รัน `run:` ด้วย **`bash -e`** อยู่แล้ว
การเติม `set -o pipefail` ทำให้ exit ของ **ทุกคำสั่งในท่อ** มีสิทธิ์ฆ่าขั้นนั้น —
รวมถึงคำสั่งที่เราใช้แค่นับของ
