...
> ### ★★ ประตูต้อง **บังคับให้สถานะที่นาน ๆ โผล่ ถูกวาด** ไม่ใช่ตรวจสิ่งที่บังเอิญอยู่บนจอ
> 7 ก.ย. 2026: ช่องบนแถบสถานะของตัวจับ I-1 โผล่เฉพาะตอนเกินเพดาน
> → ถ้าประตูตรวจแค่หน้าจอสภาพปกติ **มันจะไม่เคยเห็นข้อความนั้นเลย
> จนถึงวันที่อาการเกิดจริง** ซึ่งเป็นวันที่แย่ที่สุดที่จะเจอ tofu
> → ประตูต้องสั่งให้วาดสถานะนั้นเองก่อนตรวจ · ใช้กับทุกข้อความที่ขึ้นเฉพาะตอนผิดปกติ:
> toast ของ error · `Missing` · dialog ยืนยัน · ตัวเลขที่เกินเพดาน
>
> ### ★★★ และต้องวน **ทุกภาษา** (แก้ 10 ก.ย. 2026)
> ประตูเดิมตรึง `lang` ไว้ที่ไทย → **ข้อความบนแผงทุกตัวไม่เคยผ่านประตูในภาษาอังกฤษเลย**
> ทั้งที่อังกฤษคือ**ภาษาหลัก** ของ UI ตามหัวข้อนี้
> → สิ่งที่โปรแกรมประกาศว่ารองรับ ต้องถูกตรวจทุกตัว (`docs/08 §3.9 ข้อ 10`)
> — และ "ตรวจแล้ว" หมายถึงตรวจในทุกค่าของสิ่งที่เปลี่ยนผลได้ ไม่ใช่ค่าที่เผอิญตั้งไว้
...
### ★★★ ข้อ 4 — พหูพจน์: วันนี้ **เลี่ยง** ได้ แต่ต้องรู้ว่ากำลังเลี่ยง (12 ก.ย. 2026)

`"1 saved tags"` ผ่านประตู tofu ทั้งสองภาษาและผ่านทุกเทสต์ — **ไทยไม่มีพหูพจน์
ปัญหาจึงมองไม่เห็นจากฝั่งที่เราอ่านออกที่สุด** · เห็นได้ทางเดียวคือดูภาพหน้าจอจริง

**วิธีที่ใช้ตอนนี้ (ถูกสำหรับสองภาษานี้):** เรียบเรียงประโยคให้ถูกทุกจำนวน
โดยไม่ต้องมีกลไกพหูพจน์ — *"No file here matches 1 of the saved tags"*

> ### แต่นี่คือการ **เลี่ยง** ไม่ใช่การ **แก้**
> | ภาษา | จำนวนรูป |
> |---|---|
> | ไทย · จีน · ญี่ปุ่น | 1 |
> | อังกฤษ · เยอรมัน | 2 |
> | รัสเซีย · โปแลนด์ · เช็ก | **3–4** |
> | อาหรับ | **6** |
>
> เขียนเลี่ยงได้เมื่อมีสองภาษาที่ไม่ขัดกัน · **ภาษาที่สามที่มีสามรูปทำให้เลี่ยงไม่ได้อีก**
> และตอนนั้นมันจะไม่ใช่การเติมคำแปล แต่เป็นการรื้อ `text.rs` ทั้งตัว

**กฎสำหรับตอนนี้:**

1. ข้อความที่มีตัวเลข **ห้ามประกอบจากชิ้นส่วน** (`format!("{n} " + word)`)
   — ประโยคเต็มต่อหนึ่งภาษา เพราะลำดับคำและรูปคำต่างกันทุกภาษา
2. เขียนเลี่ยงได้ ให้เลี่ยง **แต่คอมเมนต์ไว้ว่านี่คือจุดที่พหูพจน์จะโผล่**
3. วันที่รับภาษาที่สาม → `text.rs` ต้องรับ `count` เป็นพารามิเตอร์ก่อนเพิ่มคำแปลใด ๆ
   **ไม่ใช่เพิ่มคำแปลก่อนแล้วค่อยมาแก้ทีหลัง**

> ★ ข้อนี้ **เจอได้ทางเดียวคือดูภาพหน้าจอจริง** — ประตูอัตโนมัติทุกบานผ่านหมด
> เป็นหลักฐานอีกชิ้นของ `docs/08 §3.9 ข้อ 5`

### ★★★ ทุกทางที่โปรเซส **จบ** ต้องมีอะไรให้ผู้ใช้เห็น (เจอจริง 20 ก.ย. 2026)

`main` คืน `Err` → Rust พิมพ์ไป **stderr ซึ่ง GUI app บน Windows ไม่มี**
→ graphics setup ล้มเมื่อไหร่ **โปรแกรมหายไปเงียบ ๆ**
ผู้ใช้ที่ไดรเวอร์เพิ่งอัปเดตกดเปิดแล้ว **ไม่เกิดอะไรขึ้นเลย**

กล่อง dialog มีอยู่แล้ว **แต่ต่อไว้กับ panic hook เท่านั้น** — และเส้นทางนี้ไม่ใช่ panic

> ### กฎ: ไล่ให้ครบว่าโปรเซสจบได้กี่ทาง แล้วตรวจทีละทาง
>
> | ทางที่จบ | ผู้ใช้เห็นอะไร |
> |---|---|
> | ปิดหน้าต่างตามปกติ | — |
> | `panic!` | dialog (มีแล้ว) |
> | **`main` คืน `Err`** | **ไม่มีอะไรเลย** ← เส้นทางที่หายไป |
> | ตายก่อนสร้างหน้าต่างได้ | ต้องมี dialog เหมือนกัน |
>
> → กฎ *"error ต้องบอกสิ่งที่เกิดขึ้น + สิ่งที่ทำได้ต่อ"* ใน `CLAUDE.md`
> **ไม่เคยถูกใช้กับเส้นทางเริ่มโปรแกรม** เพราะมันไม่เคยไปถึง UI
> → ★ "ไม่มี UI ให้แสดง" ไม่ใช่เหตุผลที่จะเงียบ — **มันคือเหตุผลที่ต้องใช้ native dialog**
> → ต้องบอกที่อยู่ของ log ด้วย ไม่งั้นผู้ใช้ไม่มีอะไรส่งกลับมาให้เราดู
...
> ### ★★★ สถานะจริง 18 ก.ย. 2026 — แผงมี **หัวข้อ `X / Y / W / H` ที่ไม่มีค่าอยู่ใต้มัน**
>
> ของจริงมีแค่ **opacity · filter · flip** · ที่เหลือในตารางยังไม่ได้สร้าง
> เจอตอนขับแอปจริงหนึ่งรอบเต็ม — **เทสต์ 1,070 ตัวไม่เห็นข้อนี้เลย**
>
> **ป้ายของสิ่งที่ยังไม่มี แย่กว่าไม่มีป้าย** — ผู้ใช้อ่านว่าโปรแกรมพัง ไม่ใช่ว่ายังไม่รองรับ
> และ `CLAUDE.md` เขียนไว้เองว่าความรู้สึก "เชื่อถือได้" มาจากรายละเอียดเล็ก ๆ แบบนี้
>
> **แบ่งสำหรับ v1.0 (ตัดสิน 18 ก.ย. 2026):**
>
> | | v1.0 | หลัง v1.0 |
> |---|---|---|
> | **X/Y · W/H** | **อ่านอย่างเดียว** — แสดงค่าจริง ไม่ต้องแก้ได้ | พิมพ์แก้ได้ |
> | ชื่อไฟล์ · ขนาด px · format · path | **ต้องมี** — ข้อมูลล้วน ไม่ต้องมีตรรกะ | reveal in folder |
> | rotation | มีค่าอยู่แล้วใน shader → **แสดงอย่างเดียว** | แก้ได้ |
> | **crop · lock** | **เอาป้ายออกทั้งหมด** — เป็นฟีเจอร์ ไม่ใช่การแสดงผล | สร้างจริง |
>
> ★ เกณฑ์ที่ใช้แบ่ง: **แสดงค่าที่มีอยู่แล้ว = ถูก · ป้ายของสิ่งที่ยังไม่มี = ผิด**
> ห้ามใส่ช่องที่กดแล้วไม่เกิดอะไร แม้จะ "เตรียมไว้"
