...
## 2.6 ความเสี่ยงที่ยอมรับไว้: clipboard จอง RAM ก่อนที่เกราะจะทำงาน (28 ก.ค. 2026)

`arboard::get_image()` **ถอดรหัสและจอง RGBA จนเสร็จก่อนคืนค่า** เราจึงตรวจขนาดได้
*หลัง* การจองก้อนนั้นไปแล้วหนึ่งครั้ง — ต่างจากเส้นทางไฟล์ที่ตรวจ header ก่อน allocate เสมอ

**ทำไมยังไม่ปิด:** ปิดสนิทต้องอ่าน clipboard เองด้วย `clipboard-win` ซึ่ง
1. ไม่มีใน `docs/09` และ
2. **แก้ได้เฉพาะ Windows** — Linux ยังเดินผ่าน `arboard` เหมือนเดิม
   เกราะที่ทำงานแค่แพลตฟอร์มเดียวให้ความมั่นใจผิด ๆ ซึ่งอันตรายกว่าไม่มีเกราะ (ดู `docs/08 §3.9`)

**ขอบเขตความเสี่ยงที่ประเมินแล้ว:** ระบบปฏิบัติการถืออยู่ก่อนแล้วหนึ่งชุด การวางจึงเพิ่ม
peak ประมาณเท่าตัว · จำกัดให้**วางได้ทีละครั้ง**แล้ว จึงไม่ซ้อนกันหลายก้อน

**ทบทวนใหม่เมื่อ:** มีเหตุอื่นที่ทำให้ต้องเขียน clipboard เองต่อแพลตฟอร์มอยู่แล้ว
หรือมีรายงานจากผู้ใช้จริงว่าเกิดปัญหา — **ห้ามเพิ่ม `clipboard-win` เพื่อเรื่องนี้อย่างเดียว**
...
> ### ★★★ แก้ 28 ส.ค. 2026 — ตรวจแล้วว่ากฎ "root เท่านั้น" ใช้จริงไม่ได้
>
> `validate_path` **ไม่มีอยู่จริงในโค้ดเลย** (`validate.rs` เป็น TODO บรรทัดเดียว)
> ทั้งที่ `docs/08 §4` ข้อ 9 ทำให้มันเป็นเงื่อนไข merge · ไม่มีเทสต์ traversal สักตัว
> และบนแอปจริง **RefX อ่าน `C:/Windows/win.ini` ได้** ถ้า `.refx` สั่งให้อ่าน
>
> **แต่กฎเดิมสองข้อทำตามไม่ได้:**
>
> | กฎเดิม | ทำไมไม่ได้ |
> |---|---|
> | path ใน `.refx` ต้อง relative เท่านั้น | **แยกไม่ออกว่าไฟล์ไหน "มาจากคนอื่น"** — `.refx` บนดิสก์ก็คือ `.refx` ไม่มีเครื่องหมาย |
> | ต้องอยู่ใต้ root ที่ผู้ใช้เปิดเอง | linked mode ชี้ไปที่ภาพซึ่ง**อยู่คนละที่กับเอกสารเป็นปกติ** (`D:/refs/` กับ board ที่ `C:/work/`) → บังคับ root = ภาพทุกใบกลายเป็น `Missing` ทุกครั้งที่เปิด = **พังฟีเจอร์ ไม่ใช่ป้องกัน** |
>
> **สิ่งที่ต้องกันจริง ๆ แคบกว่านั้นมาก** เพราะ I-8 ห้าม network อยู่แล้ว
> จึง**ไม่มีทางส่งข้อมูลออก** การอ่านไฟล์ในเครื่องที่ไม่ถูกแสดงผลจึงเสียหายจำกัด
>
> | ต้องปฏิเสธ | เหตุผล |
> |---|---|
> | **UNC (`\\server\share`)** | ทำให้เกิดการเชื่อมต่อออกนอกเครื่อง = **ละเมิด I-8 โดยตรง** และรั่ว NTLM hash บน Windows |
> | **device namespace (`\\.\`, `\\?\`)** · ชื่อสงวน (`CON` `PRN` `NUL` `COM1`…) | ค้างได้ · มี side effect · ไม่ใช่ไฟล์ภาพแน่นอน |
> | **`..` (`Component::ParentDir`)** | ไม่มีเหตุผลชอบธรรมใน path ที่มาจากไฟล์ |
>
> path ในเครื่องที่เหลือ **อนุญาต** — มันเดินผ่าน `decode_guarded` ซึ่งมีเกราะครบและถูก fuzz อยู่แล้ว
>
> **ลายเซ็นใหม่:** `validate_asset_path(&Path) -> Result<(), SecError>` — ไม่มี `roots`
> เรียกทุกจุดที่ path มาจาก **ไฟล์** ไม่ใช่จากการที่ผู้ใช้เลือกเอง
>
> ★ ต้องมีเทสต์ว่า `.refx` ที่มี UNC path **ไม่ทำให้เกิด TCP ออกนอกเครื่องเลย** — ผูกกับ I-8

### ★★★ ด่านที่สอง: `validate_save_target()` — สำหรับ path ที่ **ผู้ใช้เลือกเอง** (ตัดสิน 8 ก.ย. 2026)

สเปกขัดกันเองและถูกจับได้ตอน P5-4: `docs/07 §6` สั่งให้ path ของ export ผ่าน
`validate_asset_path()` แต่หัวโมดูลของมันเขียนไว้เองว่าเรียกเฉพาะ path ที่มาจาก**ไฟล์**
→ ถ้าเรียก จะ **ปฏิเสธการ export ลงไดรฟ์ที่แชร์ไว้** ซึ่งเป็นงานประจำของทีม

> ### อันตรายของ UNC ไม่ใช่ "มันคือเครือข่าย" แต่คือ **"ใครเป็นคนเลือก path นั้น"**
>
> | ที่มาของ path | UNC | เหตุผล |
> |---|---|---|
> | **ไฟล์** (`.refx` ที่เปิดมา) | **ปฏิเสธ** | ผู้โจมตีเป็นคนเลือก → บังคับให้เครื่องยืนยันตัวตนออกไปข้างนอก (NTLM hash) โดยผู้ใช้ไม่รู้ |
> | **ผู้ใช้** (กล่องบันทึกไฟล์) | **อนุญาต** | เขาเดินไปที่นั่นเอง เห็นกับตา และตั้งใจ |
>
> **ข้อนี้ไม่ขัด I-8** — I-8 ห้าม *โค้ดของเรา* เปิดการเชื่อมต่อ (จึงมีประตู `cargo tree`)
> การที่ OS เขียนไฟล์ลง share ที่ผู้ใช้ชี้เอง ไม่ใช่เครือข่ายของเรา
> **เขียนไว้ตรงนี้เพื่อกันไม่ให้ใครมา "แก้กลับ" ในอนาคต**

```rust
validate_save_target(&Path) -> Result<(), SecError>
```

| ต้องปฏิเสธ | เหตุผล |
|---|---|
| **ชื่อสงวน** (`NUL` `CON` `PRN` `AUX` `COM1`–`COM9` `LPT1`–`LPT9` — มีหรือไม่มีนามสกุลก็ตาม) | เขียนลง `NUL.png` **สำเร็จแล้วทิ้งข้อมูลเงียบ ๆ** ผู้ใช้เชื่อว่า export แล้วแต่ไม่มีอะไรเลย = งานหายแบบที่ I-3 ห้าม · **ไม่มีใครตั้งใจตั้งชื่อนี้** |
| **device namespace** (`\\.\`, `\\?\`) | ไม่ใช่ไฟล์ · มี side effect |
| UNC · ไดรฟ์อื่น · นอกโฟลเดอร์เอกสาร | **อนุญาต** — ผู้ใช้เลือกเอง |

`..` ไม่ต้องตรวจ — กล่องบันทึกไฟล์คืน path ที่คลี่แล้ว

> ### ★★ หลังจาก rename ต้อง **ยืนยันว่าไฟล์ลงจริง**
> อ่านขนาดกลับมาเทียบกับจำนวนไบต์ที่เขียน · การเขียนลง share ล้มเหลวในแบบที่
> ดิสก์ในเครื่องไม่ล้ม (ลิงก์หลุดกลางคัน · โควตาเต็ม · สิทธิ์หาย) และบางทาง
> **รายงานว่าสำเร็จ** → ไม่ตรง = แจ้งว่า export ไม่สำเร็จ ไม่ใช่เงียบ
