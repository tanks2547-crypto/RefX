...
basic-toml  = "0.1"            # อ่าน settings.toml / keymap.toml (คอมเมนต์เดิมผิด — นั่นคือ `directories`)

# --- export ---------------------------------------------------------------
# ★ เพิ่ม 7 ก.ย. 2026 (P5-4) — `image` ไม่มี API สตริมสำหรับ PNG
#   `png 0.18.1` อยู่ใน Cargo.lock อยู่แล้วในฐานะ dep ของ `image` เอง
#   → ประกาศตรง = ไม่เพิ่มโค้ดเข้า binary แม้แต่ไบต์เดียว ไม่เพิ่ม license ใหม่
#   ★★ ต้องตรึงให้ตรงกับที่ `image` ใช้ — ดูประตูบังคับด้านล่าง
png         = "0.18"           # `Writer::stream_writer` — เขียน PNG ทีละแถบ
...
### ★★★ `png` — เหตุผล และประตูที่ห้ามหาย (อนุมัติ 7 ก.ย. 2026)

**ทำไมต้องเพิ่ม:** ถ้าไม่เพิ่ม PNG จำกัดที่ ~**6000²** ซึ่ง**ต่ำกว่าเพดาน 16384
ที่ `docs/07 §6` ประกาศไว้เอง** → สเปกกลายเป็นสิ่งที่ทำไม่ได้แบบเงียบ ๆ
และ PNG คือรูปแบบที่นักวาดใช้จริงเมื่อต้องการภาพที่ไม่สูญเสีย

**ทำไมถึงถูกอนุมัติง่ายกว่ารายการอื่น:** มันอยู่ในทรีอยู่แล้ว ไม่ใช่โค้ดใหม่
ไม่ใช่ผู้ดูแลรายใหม่ ไม่ใช่ license ใหม่ · สิ่งที่เปลี่ยนคือ**เราเห็นมัน**
แทนที่จะพึ่งของที่ `image` เลือกให้โดยเราไม่รู้ตัว

> ### ประตูบังคับ — เพิ่มเข้า `docs/08 §4`
> ```bash
> cargo tree -d | grep '^png'   # ต้องว่าง
> ```
> ถ้าเวอร์ชันที่เราตรึงเลื่อนออกจากที่ `image` ใช้ จะได้ **png สองก๊อปปี้ในทรี** —
> binary โต และมีตัวเข้ารหัสสองตัวที่อาจให้ผลไม่เหมือนกัน
> รูปแบบเดียวกับประตู `cargo tree -d | grep '^wgpu'` ที่มีอยู่แล้ว
> **`^` สำคัญ** — ไม่งั้นจับ crate ลูกที่ชื่อลงท้ายด้วย png ติดมาด้วย
