...
### ★ `QuadInstance` ยังต้องเป็น 64 ไบต์ — ให้ทวงที่คืนจาก `tint` (ตัดสิน 4 ส.ค. 2026)

P2-8 ต้องการที่เก็บ `brightness`/`contrast` แต่ 64 ไบต์เต็มแล้ว
ทางแก้ชั่วคราวคือยัดลง 16 บิตบนของ `flags` (ละเอียด 1/127 ต่อขั้น)

**ไม่ขยายเป็น 80 ไบต์** และ**ไม่ต้องทนกับ 1/127** — ที่เสียเปล่าอยู่ตรงหน้าแล้ว:

`tint: [f32; 4]` = **16 ไบต์** เพื่อเก็บตัวคูณสีในช่วง 0..1
ปลายทางคือ framebuffer 8 บิตต่อช่อง → **f32 ให้ความละเอียดที่มองไม่เห็นความต่าง**

→ เปลี่ยนเป็น `[u8; 4]` (`VertexFormat::Unorm8x4` — GPU ขยายเป็น 0..1 ให้ฟรี)
→ **คืนที่มา 12 ไบต์** พอสำหรับ `brightness` + `contrast` เป็น f32 เต็ม (8 ไบต์) เหลืออีก 4
...
| `tint` → `[u8; 4]` | **4** |
| `layer` · `flags` | 8 |
| `brightness` · `contrast` (f32) | **8** |
| เผื่อไว้ | 4 |
| **รวม** | **64** |

ได้ทั้งความละเอียดเต็ม · งบ VRAM ใน §3 ไม่ต้องคำนวณใหม่ · ยังพอดีหนึ่ง cache line

> **ข้อจำกัดที่ยอมรับ:** `tint` จะเกิน 1.0 ไม่ได้อีก (ไม่มี HDR multiply)
> ซึ่งไม่กระทบอะไรเพราะการทำให้สว่างกว่าต้นฉบับเป็นหน้าที่ของ `brightness` ไม่ใช่ `tint`
> ถ้าวันหนึ่งต้องการ tint แบบ HDR จริง ให้กลับมาคุยเรื่องขยาย struct ตอนนั้น
