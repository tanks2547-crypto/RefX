...
///
/// ★ ใช้ตัดสินใจก่อนทำอย่างอื่นเสมอ: ไฟล์รุ่นใหม่กว่าต้องรู้ตัวตั้งแต่ 20 ไบต์แรก
...
pub mod v1 {
    ...
    pub struct AssetDto {
        ...
        /// เส้นทางล่าสุดที่เจอ
        ///
        /// ★★ **เป็น `String` ไม่ใช่ไบต์ดิบของ OS — และนั่นยอมรับได้**
        ///
        /// path ที่ไม่ใช่ UTF-8 (เกิดได้บน Linux, แทบไม่เกิดบน Windows ซึ่งเป็น
        /// เป้าหมายหลัก) จะถูก `to_string_lossy` แปลงเพี้ยน · เหตุผลที่ยอมได้
        /// **ไม่ใช่แค่ว่ามันหายาก**:
        ///
        /// `docs/02 §2.3` กำหนดว่า **`hash` คือคีย์หลัก ส่วน `path` เป็นแค่ hint**
        /// → path ที่เพี้ยนไม่ได้ทำให้ภาพหาย · relink (docs/07 §2) ยังหาไฟล์เจอ
        /// ด้วย hash ทั้งในโฟลเดอร์เดียวกับ `.refx` และจากตาราง `paths` ใน cache
        /// · ที่เสียไปคือ "เดาที่อยู่เดิมได้เร็วขึ้น" ซึ่งเป็นความสะดวก ไม่ใช่ข้อมูล
        ///
        /// ★ เก็บเป็นไบต์ดิบจะต้องมีเส้นทางต่างกันระหว่าง Unix (`OsStrExt`) กับ
        /// Windows (UTF-16) แล้วไฟล์จะ **ย้ายข้าม OS ไม่ได้** ซึ่งขัดกับเจตนาของ
        /// packed mode ที่มีไว้ส่งให้คนอื่น (docs/07 §2)
        /// → **อย่าเปลี่ยนเป็นไบต์โดยไม่มีเหตุผลใหม่ที่หนักกว่านี้**
        pub path: String,
...
mod tests {
    ...
    fn a_file_from_a_newer_build_is_never_overwritten() {
        ...
        // ★★ ข้อความต้องบอก **ทั้งสองเลข** — "ไฟล์นี้ใหม่เกินไป" อย่างเดียว
        //    ไม่บอกผู้ใช้ว่าต้องทำอะไรต่อ · เขารู้ว่าไฟล์เป็น v เท่าไหร่ และรุ่นที่
        //    เขามีอ่านได้ถึง v เท่าไหร่ จึงจะรู้ว่า "ต้องอัปเดตโปรแกรม" ไม่ใช่
        //    "ไฟล์พัง" (docs/07 §1 — สองอย่างนี้เป็นคนละการกระทำ)
        let message = OpenError::NewerVersion {
            found: 7,
            understood: FORMAT_VERSION,
        }
        .to_string();
        assert!(message.contains("v7"), "ไม่บอกว่าไฟล์เป็นเวอร์ชันอะไร: {message}");
        assert!(
            message.contains(&format!("v{FORMAT_VERSION}")),
            "ไม่บอกว่ารุ่นนี้อ่านได้ถึงเวอร์ชันอะไร: {message}"
        );
