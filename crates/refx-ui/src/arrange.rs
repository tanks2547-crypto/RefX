...
mod tests {
    ...
    fn a_hundred_mode_switches_leave_the_board_exactly_as_it_was() {
        ...
        // ★★ **ขอบเขตที่เทสต์นี้ครอบไม่ถึง — ต้องรู้ไว้** (เพิ่มตอน P4-1)
        //
        //    ตั้งแต่ P4-1 ชั้น `app` เขียน `Board::set_view()` ทุกเฟรม ซึ่งทำให้
        //    `Board.view.mode` **เปลี่ยนจริงตอนสลับโหมด** · นั่นถูกต้องตาม
        //    docs/02 §2.9 (`view` persist ลงไฟล์ แต่ยกเว้นจาก `Command`/`dirty`)
        //    และไม่ขัด docs/03 §4.3 ที่ห้ามแค่ "สร้าง Command / ทำให้ dirty"
        //
        //    เทสต์นี้ไม่ขับ `App` จึงไม่มีใครเรียก `set_view` → `board == before`
        //    ยังจริงอยู่ · แต่สิ่งที่มันพิสูจน์คือ **"เนื้อหาไม่เปลี่ยน"**
        //    ไม่ใช่ "ทุกไบต์ใน Board ไม่เปลี่ยน" — ถ้าวันหนึ่งย้ายการซิงค์ลงมา
        //    ชั้นนี้ ต้องเทียบแบบยกเว้น `view` ไม่ใช่ปิดเทสต์ทิ้ง
